...
3. **Refresh this machine's live skill links — every push, never judged.** Run `bin/sync-cross-tool` after the push. It is idempotent and reconciles the flat symlink mirrors Cursor (`~/.cursor/skills/`) and Codex (`~/.agents/skills/` — the location current Codex scans; `~/.codex/skills/` is legacy and no longer read) use; ordinary project-local `.claude/skills/` remain Claude-only unless a skill opts in with `harness-targets:`. Existing links point into this repo, so edited content is already live through them; the run is what picks up added, renamed, removed, or retargeted skills — run it every time rather than judging whether the diff "needed" it (the kitchen's skills once sat unmirrored for Codex because the destination had gone stale and nothing re-checked it). Restart or begin a new Cursor/Codex session so its skill catalog is rebuilt.
...
5. **Refresh Codex — when the push touched `skills/`, `.codex-plugin/`, `.agents/plugins/`, or README.md.** Run `codex plugin marketplace upgrade claudia-kitchen`. If the `codex` CLI is missing or the command fails, report that plainly instead of silently dropping the step. Start a new Codex thread afterward; the current thread keeps its existing skill catalog.
