...
when_to_use: |
  ...
  Scoped to this repo's STATE.md only — a curated snapshot of Claude Code plus the coding-agent landscape. Not a general changelog summarizer.
---

<!-- Earned against: Opus 4.7 (claude-opus-4-7), 2026-05-24. Re-tested 2026-05-29 (Opus 4.8): KEPT, slim held. The generic filter instinct is now largely native (a skill-withheld trial filtered 4 of 16 changelog entries and used a header-only dateline), but the repo-specific section map + dateline convention aren't inferable — that is the load-bearing part. Did not cut teaching content: n=2, skill is 5 days old. -->
...
The default failure is bulk paste — dropping every new entry into a fresh "What's new" section, breaking cadence, and reintroducing inline datelines that the header already subsumes. STATE.md states its filter on line 5: _what changes how you build, configure, and ship_. Most changelog entries don't pass it.
...
3. **Does it contradict something already in STATE.md?** Renames, flipped defaults, retired features — the existing line is now wrong and must be **edited in place**, not appended around.

Most entries don't pass (1). Accept that. STATE.md is filtered, not exhaustive — the changelog is already the exhaustive source.
...
The table maps by _kind_; treat it as a default, not a rule. When an entry speaks more directly to a concern a section already owns, place it there instead — e.g. a settings key that's really a description-budget lever belongs in _The cost model nobody tells you about up front_'s practical-consequences list, not under _Other small but useful_ just because it's a setting.

If a surviving entry has no natural home, that's a signal — either the entry doesn't actually pass triage, or the section structure has drifted from the surface. Ask before adding a new section.
...
- **Re-pin both datelines**: line 3 (`Claude Code vX.Y.Z … verified YYYY-MM-DD`) and the footer ("re-checked YYYY-MM-DD against vX.Y.Z"). HACKS.md may be pinned several versions behind STATE.md — its delta starts at *its* pin, not STATE.md's.
...
- **Add only hack-worthy entries**, as table rows or single ⚠️/💡 lines in the existing sections. It's a scannable table/glossary — no prose paragraphs (see auto-memory `hacks-md-form`).

## Step 6 — Check CLAUDE.md for staleness (standing)

Scan the repo CLAUDE.md against the new state: any rule, footgun, or pin the release contradicts? CLAUDE.md holds intent, not current state, so most bumps find nothing — but a contradicted line must be flagged to the user, not silently relied on. A major *model* release additionally activates its model-version-pinning section: every `Earned against:` pin in the repo becomes a re-test candidate. Where a kitchen artifact carries `evals/probes.md`, the re-test is a replay of that file — inventory the probe files alongside the pins and offer the runs (real tokens, opt-in). The applied skills' replay is owned by the skills repo: hand off to its `/retest` skill (`.claude/skills/retest/` in the sibling `skills` checkout) rather than reconstructing the procedure here; if that repo isn't checked out, flag the pending re-test instead of improvising one.

## Step 7 — Sweep the kitchen's skills/ artifacts (standing)

A bump catalogues new harness capabilities — the trigger to check whether any forge or applied skill now contradicts STATE.md, is missing a lever it should name, or could improve. Run this pass on every substantive bump (skip on a date-only bump):

- **Scope to harness-coupled artifacts only** — the kitchen's `skills/` (forge, harness-audit, harvest, ingest — they design against Claude Code surfaces). The applied skills live in the separate `skills` repo; if it's checked out, sweep its harness-coupled skills there too. **Exclude** stack/writing skills (UI, design, prose): a Claude Code changelog can't make "use oklch not hex" stale.
...
If the user says "just bump the date" without a substantive review, two options — surface them:
