...
The skill loader executes two literal byte sequences as shell commands in any file inside a skill directory. Markdown context (fences, inline code, quotes) offers no protection, and a hit breaks the whole skill's loading. The sequences, how to document them safely, and the self-check grep are in the `new-skill` skill. `scripts/preship-check` catches both. `scripts/preship-gate` is the harness-agnostic PreToolUse wrapper that runs it on every `git commit` and blocks on failure; it resolves the repo root itself and reads the command out of either payload shape, so one script serves every harness that speaks the PreToolUse contract. Claude Code wires it in the committed `.claude/settings.json`, and there it is verified to fire, so there is no need to run the check manually. Codex is wired in `.codex/hooks.json` but is **not** verified: on 0.147.0 a project-scoped `.codex/hooks.json` produced no hook execution at all, not even for events that demonstrably fire from other sources, so on Codex run `scripts/preship-check` yourself before committing until `/hooks` shows the project entry loaded and trusted. Treat any harness whose gate has not been observed blocking a real commit the same way.

## Publishing footgun: Claude stays versionless; Codex does not

No `version` field belongs in either plugin's `.claude-plugin/plugin.json` or the Claude `claudia` marketplace entries. Commit-SHA versioning is what makes a pushed commit propagate on the next `/plugin update`; a version string there pins Claude's per-install cache and pushed changes silently stop reaching other repos (a frozen `0.1.0` hid `quality-audit` for weeks). `scripts/preship-check` fails if one reappears.
