...
- The **primary deletion signal is live convergence**: mid-task, an artifact's guidance and the model's bare instinct agree — when that happens, log the artifact as a deletion candidate in CHANGELOG.md. On each major model release (or newly adopted working model), replay each artifact's `evals/probes.md` (prose reconstruction for artifacts that predate their probe file) — the scheduled sweep for what live use hasn't surfaced, and the confirmation before any candidate is actually deleted. Log verdicts (KEPT / revised / deleted) in CHANGELOG.md. Delete the ones whose gap no longer reproduces; rewrite the ones whose gap has shifted. A model can also be **withdrawn**, not just superseded — a pin to an unreachable model is a dead trigger; re-pin those artifacts to the period's durable default and record the withdrawal in CHANGELOG.md.
