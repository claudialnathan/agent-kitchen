...
<tr><td>plugin CLI + state</td><td><code>bin/</code> at plugin root · <code>${CLAUDE_PLUGIN_DATA}</code></td><td><code>bin/</code> joins the Bash <code>PATH</code> while enabled (ship a CLI your skills call by name) — <strong>CLI only</strong>: claude.ai-hosted sync rejects any plugin with a top-level <code>bin/</code>, failing the whole marketplace behind a generic "sync failed" (real error in <code>~/Library/Logs/Claude/claude.ai-web.log</code>). <code>${CLAUDE_PLUGIN_DATA}</code> is the persistent state dir that survives updates (install venvs / <code>node_modules</code> there once).</td></tr>
