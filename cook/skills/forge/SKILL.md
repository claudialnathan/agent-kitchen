...
1. **It encodes expertise the model lacks — it beats the default.** A frontier model's unaided output is competent across most domains; restating that competence is dead weight at recurring cost. The artifact earns its place *only* with the delta a senior practitioner adds and the model would not produce unprompted: the non-obvious move, the trade-off and its reasoning, the failure mode, the verified current specific. This bar decides whether the work is worth doing at all — and it is the one most easily faked with confident-sounding floor, so verify the delta as an action, not an intention: for anything fast-moving or version-specific, fetch the canonical source and cite it; recall ships deprecated specifics in a confident tone. The delta has only a few durable sources — the owner's taste made operational, local truths of a repo or team the model cannot know, verified post-cutoff currency, and failures actually observed. An artifact drawing on none of these is competing with the model's own training data and will be absorbed within a release or two; write it expecting to delete it.
2. **It is earned.** A failure you observed (the agent did the wrong thing, or the same correction landed three times — the `harvest` skill mines session transcripts for exactly these), or an authoritative source worth operationalizing before the failure happens — a spec, a researched methodology, hard-won domain knowledge, not a hot take. If neither, stop: the cheapest artifact is the one not written.
...
- **Encode the discovery, not the facts.** When local truth exists at runtime — a token file, a config, a schema — teach the skill to read it first instead of freezing today's values ("read `globals.css` for the theme before writing classNames"). Frozen local facts are the fastest-rotting content a skill can carry; the discovery survives every project the snapshot dies in.
...
- **Ship the death test with the skill.** Expertise and taste skills carry `evals/probes.md` — the earning failures as runnable fixtures: verbatim prompt, unaided-failure signature, pass criterion, per-model baseline verdicts. Deletion needs *all* probes passing unaided; models absorb the easy case first, and a single-probe pass once nearly killed a still-earning skill. Process and workflow skills are exempt — their value is the owner wanting the procedure, which a release can't absorb. Mechanics: [references/skills.md](references/skills.md).
- **On each major model release (or newly adopted working model), replay each artifact's probe file** — prose reconstruction only where none exists yet. No longer reproduces → delete; shifted → rewrite against the new failure rather than layering on the old. And record the keeps: when a skill-withheld trial reproduces the failure, that's a dated KEPT in the baseline table and the changelog — without positive evidence, every audit can only ever argue for removal.
