...
- **Test against the failure that earned it.** Run the task in a fresh session *without* the skill; the gap is the spec. Then with it — and read the transcript, not just the output: "didn't trigger" and "triggered but didn't steer" look identical from outside. Scale verification to stakes; three opt-in harnesses back this (real tokens — confirm before firing), cheapest first: `evals/trigger-eval.js`, a text proxy that A/Bs a *description*'s discriminating power (a judge guesses fire/no-fire); `evals/invocation-eval.js`, the **live ground truth** that drives a real `claude -p` and watches whether the skill actually fires (run with `node`, not Workflow — it shells out; trust it over the proxy when a fire/no-fire verdict must hold); and `evals/depth-eval.js`, a blind A/B panel for whether the *body* steers toward expert work. Proxy and live can disagree — a model often handles a matching prompt inline rather than load the skill, which only the live run sees.
