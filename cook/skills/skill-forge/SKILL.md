...
when_to_use: |
  Any work on a skill: creating one, distilling an article or doc into one, capturing a workflow the user keeps retyping, reviewing or refactoring one, fixing triggering, or deciding whether a skill is the right surface against a hook, rule, subagent, or CLAUDE.md. Also packaging a /command or plugin, and operationalizing collated sources.
...
- **Distillation's blast radius is one skill.** Ingesting material into a skill never adds content or back-references to a neighboring skill. If an existing skill covers adjacent ground, the new one stays self-contained and at most carries a soft, optional, one-directional pointer ("if `x` is available, use it alongside this"). A skill must do its job alone; coupling two so each leans on the other breaks both the moment one is missing. <!-- Earned against: Fable 5, 2026-06-10, v2.1.170 -->
...
2. **Describe the class of requests, then anchor it.** Triggering is semantic, not lexical — the model reads the listing and matches meaning, so a statement of the request class ("any request to write or edit prose someone will read") covers ground a list of quoted phrasings only samples. Misses happen at the boundary, and an enumerated phrase list reads as exhaustive: the unlisted case looks out of scope. Add verbatim phrases only for signal the class doesn't imply — the skill's name and aliases, goal phrasings ("make this less AI-sounding"), wording that disambiguates sibling skills. Add a _negative_ trigger only when the skill demonstrably over-fires.
