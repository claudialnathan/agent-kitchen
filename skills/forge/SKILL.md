...
Do not load the shared snapshots by default. When model/effort/eval choice changes how an artifact should be authored or earned, read the plugin-root `MODELS.md`; when surface choice depends on a current harness capability, read the plugin-root `STATE.md`. Resolve them from this skill's source, not the caller's working directory. Both are orientation only: verify decision-bearing claims live, and proceed normally if either file is absent.
...
Provenance lives in the changelog, never inside the artifact: rationale, verdicts, and revisions go to CHANGELOG.md, and the artifact never references the conversation that produced it (full discipline: the kitchen's CLAUDE.md). Don't stamp an artifact with the model or harness version it was written against — that marker rots the week it's written and reads as a guarantee to anyone running a different model.
...
- **The verdict is the output against the objective at an expert bar, not whether it fired.** Establish the exemplar first: what a senior practitioner's output on a real task in this domain looks like. Then state the gap concretely — the objective, what the model does by default in the place the artifact will deploy, and what you want instead — because an artifact whose gap you can't articulate in those three terms has nothing to grade. Judge the default where the work actually happens, not in a clean session: context that steers the model below its own knowledge is the gap for counterweight artifacts, and stripping it away measures what the model knows rather than what it does. Triggering and anatomy are pass/fail hygiene; depth is the grade. Calibrate the bar to the skill's kind: expertise grades against the domain's best practitioner; *taste* (a voice, an aesthetic) grades against its owner, where a generic judge panel is a category error. Testing a draft on a real task before shipping is good practice, not a required artifact — and read the transcript, not just the result, because "didn't trigger" and "triggered but didn't steer" look identical from outside. Optional harnesses: [references/skills.md](references/skills.md).
