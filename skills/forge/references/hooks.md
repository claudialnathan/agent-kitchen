...
Common input on every hook: `session_id`, `transcript_path`, `cwd`, `permission_mode`, `effort.level` (also exported as `$CLAUDE_EFFORT`, so gate expensive checks on it), `hook_event_name`, plus event-specific fields.
...
| `http` | Policy lives in an external service | `command` would do (it adds latency + a failure mode). Non-2xx = non-blocking; use `allowedEnvVars` for header secrets |
...
| `agent` | Multi-step verification with tool access, low-frequency events | Almost always, because most "agent" cases are `prompt` or `command` |

Common handler fields: `if` (permission-rule filter like `"Bash(git *)"`), `timeout` (defaults 600s command/http/mcp, 30s prompt, 60s agent), `statusMessage`, `once` (skill-frontmatter only). On `command`, an `args` array spawns the executable directly with no shell, so prefer it when paths could contain spaces.

`command` discipline: read JSON from stdin (`INPUT=$(cat)`), extract with `jq -r '.tool_input.file_path // empty'`, exit 2 + stderr to block, exit 0 silent to pass. Keep it fast, because hooks fire synchronously; sub-100ms is the target for per-tool-call hooks. Path vars: `${CLAUDE_PROJECT_DIR}`, `${CLAUDE_PLUGIN_ROOT}`, `${CLAUDE_SKILL_DIR}` (frontmatter hooks).
...
A frontmatter hook is auto-scoped to its artifact's lifetime, so there is no need for the script to gate itself. This dissolves the hook-or-skill either/or: a skill can carry its own enforcement.
