...
Testing a draft is authoring judgment, not a shipping gate, and it produces no artifact: notes go to CHANGELOG.md, never to an `evals/` directory or a comment inside the skill.

State the gap in three terms before writing anything — the objective, what the model does by default where the artifact will deploy, and what you want instead. Then write the minimal instruction that closes it and try it on two or three *real* prompts, the wording you'd actually type. Read the transcript, not just the result: "didn't trigger" and "triggered but didn't steer" are indistinguishable from the output alone.

Judge the default in the place the work happens. A clean session measures what the model knows; an artifact often exists because something in context — a below-bar codebase, a misleading doc — holds deployed behavior under that. Strip the steering and a counterweight artifact looks unnecessary while it is still earning its place.

Three opt-in harnesses back this when stakes are high. All cost real tokens; confirm before firing.

| Harness | Question | Notes |
| :--- | :--- | :--- |
| `evals/depth-eval.js` | Does the body steer toward expert work? | Blind 3-lens A/B panel, ~0.3–1.1M tokens/domain. Blind because a model scores its own output higher unblinded |
| `evals/trigger-eval.js` | Does the description discriminate? | Cheap text proxy: a judge scores discriminating power over a labeled query set (should-fire vs near-miss). Can A/B `description` against `description + when_to_use` to size cross-tool loss |
| `evals/invocation-eval.js` | Does the model actually invoke it? | Live ground truth: installs the real skill and drives one `claude -p` per query. Run with `node`, not the Workflow tool, because it shells out to the CLI |
...
**Cost discipline for anything that shells out to the CLI (2026-07-08 incident).** Each headless `claude -p` turn spends real tokens against the *same* session limit as your interactive session. A parallel batch of them drained a Max plan in one wave, then drained the reset limit in under a minute on relaunch. Run these sequentially, cap concurrency at 2, stop on the first session-limit error, and never fan a whole query set out with `xargs -P`.
