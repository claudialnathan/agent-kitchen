...
| `context: fork` + `agent` | Forked research. The body becomes the subagent's *prompt*, so a body with no actionable task returns empty. `Explore` for read-only, `general-purpose` when it needs Edit/Write |
...
- Nested spawning (a subagent whose `tools` include `Agent` spawning its own subagents) is **opt-in, off by default** (reversed at v2.1.217 from the default-on behavior introduced v2.1.172): set `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` to the number of layers wanted. While off, `Agent` stays listed on every subagent but errors instead of spawning; omit `Agent` or set `disallowedTools: [Agent]` if you want the leaf-only behavior to be explicit rather than incidental. No fixed depth cap now — depth is whatever you configure. A fork still can't spawn a fork. Three separate session-wide caps also apply regardless of nesting: 200 subagents total (`CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION`), 20 running concurrently (`CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS`, exempt under workflows/`ultracode`), 200 WebSearch calls. (2026-07-24, v2.1.217)
- Setting both `disable-model-invocation: true` and `paths:` (auto-loading is disabled by the former; pick one).
- Don't copy frontmatter from the skill you're reading: this forge's `paths:` globs (`**/.claude/skills/**`, `**/CLAUDE.md`) match harness files, not a stack skill's source, so copy the field idea, not the values.
...
- **Compaction** re-attaches the most recent invocation of each skill: first **5,000 tokens** per skill, shared **25,000-token cap**, filled most-recently-invoked first, and older invocations drop entirely. A load-bearing skill that got dropped: just re-invoke it.
...
- Write the body as **standing instructions** ("when running tests, prefer single-file runs"), not one-time steps ("first check Node is installed"), because the turn-8 agent should still be steered, not stepped through a spent recipe. Long bodies dilute their own steering; push reference material to sibling files.
...
The loop is eval-first: run the task in a fresh session **without** the skill and document exactly where the output falls short of the artifact's objective. That gap is the spec. Write the minimal instruction that closes it; re-test on two or three *real* prompts (the wording you'd actually type), and read the transcript for the steered-vs-triggered distinction. Three opt-in harnesses back this when stakes are high; all cost real tokens, so confirm before firing. For *does the body steer toward expert work*: `evals/depth-eval.js`, a blind 3-lens A/B panel (~0.3–1.1M tokens/domain) (blind, because a model scores its own output higher unblinded). For *does it trigger*, two fidelities: `evals/trigger-eval.js` is the cheap text proxy, where a judge scores a description's discriminating power over a labeled query set (should-fire vs near-miss), and can A/B `description` against `description + when_to_use` to size cross-tool loss; `evals/invocation-eval.js` is the live ground truth, which installs the real skill and drives an actual `claude -p` per query to watch whether the model invokes it unprompted (run with `node`, not the Workflow tool, because it shells out to the CLI). Prefer the live run when a verdict must hold: the proxy can score full recall while the real model, handed a matching prompt, just does the task inline, and only the live run sees that. **Probe replay batches** use `bin/run-probes` (sequential by default, `--confirm` required). Never parallelize a full `evals/probes.md` manifest.

**Probe files** (`evals/probes.md`) persist the earning gap as runnable fixtures. Per probe: the verbatim prompt, the unaided default it diverges from, the pass criterion (derived from the artifact's objective); plus a per-model baseline verdict table and the all-probes deletion rule. Unaided baseline runs use `claude --safe-mode` in a scratch project, because path-triggered auto-load contaminates the baseline otherwise (and `--safe-mode` still leaks account-level context, so treat the baseline as near-clean, not clean-room). Safe mode strips the harness, not the gap: when the earning gap was context-induced — the model conforming to a below-bar codebase or a misleading doc — the fixture must carry that steering material inside the scratch project, or the baseline run measures knowledge-on-request and the probe over-reports absorption. That is the commonest way a counterweight artifact gets deleted while still earning. Replay on each model release; record verdicts in the table and CHANGELOG.md. This turns the re-test from prose reconstruction into a replay, and doubles as the regression test for edits to the skill body. **Absorption is not monotonic across model lines**: a probe one model passed can fail on its successor or a sibling line (observed 2026-07-05: a case Opus 4.8 had absorbed regressed on Fable 5), so probes are never retired while the skill lives, and shrink decisions want n≥2 runs of the decisive probes, not one. Taste-skill probes carry two verdict columns (mechanical tells and owner acceptance), and divergence between them is signal: a mechanically-clean output the owner rejects means the tells list is missing one.

**Probe batch cost (2026-07-08 incident).** Each full agent replay is a live headless `claude -p` turn, with real tokens and the same session limit as your interactive session. Parallel batches drained a Max plan in one wave (see CHANGELOG 2026-07-08). **Cheap path:** `bin/grade-probe --all` grades existing artifacts with zero tokens (deterministic checks against `evals/probes.md` criteria); `bin/gen-probe --confirm --only <id>` re-runs one probe as a single `--tools ""` turn with fixture context inlined (~5–20k tokens). **Expensive path (last resort):** `bin/run-probes --confirm --only <id>` (sequential, hard cap concurrency 2, stop on session limit). Never `xargs -P 6` a full manifest. Harness: `skills/forge/evals/{grade-probe.js,gen-probe.sh,run-probes.sh}`.
