...
| Sessions run, but take contradictory steering from turn one | Steps 1 and 3. Contradictions between surfaces are the finding, not cost. |
...
| A command fails, or the CLI errors or warns at startup | Not this skill. Localizing a broken install, an invalid config, or a hook that will not load is debugging; this audit assumes the harness runs, and asks what it costs and whether it coheres. |
