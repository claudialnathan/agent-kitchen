...
- **The loader trigger sequences.** An exclamation mark immediately followed by a backtick, or three backticks immediately followed by an exclamation mark, anywhere in the skill directory, makes the loader run what follows as a shell command; a malformed or off-allowlist command fails the *whole skill* to load. Markdown context offers no protection. Grep the tree with both patterns backslash-escaped. `bin/preship-check` catches both.
