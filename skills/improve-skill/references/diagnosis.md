...
- When it does over-fire, take the negative from the request that actually arrived rather than the job that seems conceptually adjacent: read the mis-firing session and name what was really being asked. The recurring shape is a neighbouring task that shares the skill's main noun, so an audit of a harness gets reached for a harness that won't start. Name that task as its own job ("localizing a failing command is a debugging job rather than this one") instead of adding more of what the skill does cover, and suspect an earlier compression — shortening a description to a routing signature strips the boundary clause and the concrete spine before it strips the verb, and Cursor and Codex read that field and nothing else.
- Check the body for the same pull before calling it fixed. A description drawn tight still over-fires when the body confirms the wrong fit once loaded: debugging instruments sitting inside an audit skill tell the session it landed in the right place. Scope each instrument to the job it serves and add the failure signature to the skill's anti-patterns, so the redirect survives the moment the body is already in context.
