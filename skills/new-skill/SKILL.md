...
- **Name the nearest wrong job, not just the right one.** A description that says only what the artifact does gets reached for by whatever adjacent task shares its main noun: an audit skill pulled into debugging a broken config, a review skill pulled into writing the fix. Name that adjacent task as its own job in one clause ("localizing a failing command is a debugging job rather than this one"), and pick the neighbour by asking which wrong request would arrive wearing the same words, not which one is conceptually closest. Check the boundary survives every later shortening: compression strips it before it strips the verb, and on Cursor and Codex the description is the whole trigger surface.
