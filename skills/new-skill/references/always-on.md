...
**When the installed fleet collides, CLAUDE.md is the adjudicator — but only where the model routes autonomously.** A large user-scope roster ships near-duplicate skills and MCPs; the harness already lists them all, so restating availability is a mirror. What earns a line is the override the model can't infer: which skill governs a task class in this repo, where a stack boundary switches a fleet off (a mixed repo where the styling-framework skills apply on one side of the boundary and not the other), the MCP-vs-doc split (the MCP answers what the data says; the repo doc answers how the integration works). Two qualifiers do most of the filtering. *Invocation style*: in a repo whose owner invokes skills manually, the model never faces the collision, so picker rows are dead weight — what survives is the misfire guard, the row that stops an autonomously-triggered skill acting on the wrong slice, and the highest-value guard wants mechanical backing (a path-scoped rule on the slice) over prose. *Subtraction outranks annotation*: per-project `enabledPlugins` shrinks the collision space structurally before any row is written. These rows name installed artifacts by necessity — the one pointer kind that rots on a rename, not just a move — so keep them few and delete a dead row on sight; it reads as coverage while routing nowhere.
